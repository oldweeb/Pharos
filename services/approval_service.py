...
class ApprovalService:
    ...
    async def approve_token(self, account: LocalAccount, token_address: str, spender_address: str, amount: int) -> None:
        spender = self._web3.to_checksum_address(spender_address)
        token_address = self._web3.to_checksum_address(token_address)
        ...
        transaction = {
            ...
            'chainId': CHAIN_ID,
            'type': 2,
            'nonce': await self._web3.eth.get_transaction_count(account.address, 'latest')
        }
        
        gas = await GasHelper.estimate_gas(self._web3, transaction)
